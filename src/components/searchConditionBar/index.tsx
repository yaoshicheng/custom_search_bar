...
const SearchConditionResultBar = (props: SearchConditionResultProps) => {
  ...
  const resizeContainer2 = () => {
    const containerBound = (
      containerRef2.current as unknown as HTMLElement
  ...
  const renderItems = () => {
    const items = filterData.map((item) => {
      ...
      return renderItem(_transItem, onDelete);
    });
  ...
  return (
    <div className={'ysc_ux_search_condition_result_bar'} style={{ display: 'flex' }}>
      <div
        ref={containerRef2}
        style={{
          display: 'flex',
